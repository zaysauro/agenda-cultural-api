...
function weekendBounds(){
  ...
  saturday.setDate(t.getDate()+(day===0?6:6-day));
  const sunday=new Date(saturday);
  sunday.setDate(saturday.getDate()+1);
  return {saturday,sunday};
}
function renderWeekend(){
  const box=document.querySelector("#weekend-events"),empty=document.querySelector("#weekend-empty"),label=document.querySelector("#weekend-range");
  if(!box)return;
  const {saturday,sunday}=weekendBounds();
  const list=allEvents.filter(e=>{const d=parseDate(e.startDate);return d&&d>=saturday&&d<=new Date(sunday.getFullYear(),sunday.getMonth(),sunday.getDate(),23,59,59)}).sort((a,b)=>{
    const da=parseDate(a.startDate)?.getTime()||Infinity,db=parseDate(b.startDate)?.getTime()||Infinity;
...
function render(){const list=allEvents.filter(e=>inRange(e));list.sort((a,b)=>(parseDate(a.startDate)?.getTime()||Infinity)-(parseDate(b.startDate)?.getTime()||Infinity));els.events.innerHTML="";els.count.textContent=list.length+" "+(list.length===1?"evento":"eventos");els.empty.hidden=list.length!==0;for(const e of list){const f=els.template.content.cloneNode(true),p=dateParts(e.startDate);f.querySelector(".day").textContent=p.day;f.querySelector(".month").textContent=p.month;f.querySelector(".weekday").textContent=p.weekday;f.querySelector(".category-badge").textContent=categoryLabel(e.category);f.querySelector(".event-title").textContent=e.title;f.querySelector(".time").textContent="◷ "+dateText(e.startDate)+(e.startTime?" · "+e.startTime:"");f.querySelector(".venue").textContent="⌖ "+e.venue+(e.address?" · "+e.address:"");f.querySelector(".price").textContent=e.price||"Informações";const link=f.querySelector(".event-link");link.href=e.url||"#";if(!e.url)link.style.display="none";els.events.appendChild(f)}renderCalendar();renderWeekend()}
function categorySlug(value){const s=norm(value);if(s.includes("esporte")||s.includes("futebol")||s.includes("coritiba")||s.includes("athletico")||s.includes("couto pereira")||s.includes("ligga arena"))return"esporte";if(s.includes("cinema")||s.includes("filme")||s==="cine")return"cinema";if(s.includes("musica")||s.includes("show")||s.includes("concerto"))return"musica";if(s.includes("teatro")||s.includes("circo")||s.includes("espetaculo")||s.includes("peca"))return"teatro";if(s.includes("danca"))return"danca";if(s.includes("exposicao")||s.includes("mostra")||s.includes("galeria"))return"exposicao";return"cidade"}
